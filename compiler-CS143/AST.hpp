...
class Stmtnode : public ASTnode{
    ...
    virtual data_flow Data_Flow_Analysis(data_flow id_status) = 0;
...
class Programnode : public ASTnode{
    ...
    void generate_RiscV(){
        ...
        while(aryite != AryDel.end()){
            ...
            basic_environment[(*aryite)->id] = cur_off; //a2:-32, a1:-40, a0:-48
            aryite ++;
        }
        ...
        std::vector<std::unique_ptr<Stmtnode> >::iterator stmtite = stmtDel.begin();
        while(stmtite != stmtDel.end()){
            (*stmtite)->RiscV_generator(0,id_register, cur_off);
            stmtite ++;
        }
    ...
    void data_flow_analysis(){
        data_flow id_status;
        std::map<std::string, Status> basic_status;
        std::vector<std::unique_ptr<Variablenode> >::iterator ite = ValDel.begin();
        while(ite != ValDel.end()){
            basic_status[(*ite)->id] = Status(_TOP);
            ite ++;
        }
        /*
        std::vector<std::unique_ptr<Arraynode> >::iterator aryite = AryDel.begin();
        while(aryite != AryDel.end()){
            basic_status[(*aryite)->id] = Status(_TOP);
            aryite ++;
        }*/
        id_status.flow_table.push_back(basic_status);
        std::vector<std::unique_ptr<Stmtnode> >::iterator stmtite = stmtDel.begin();
        while(stmtite != stmtDel.end()){
            id_status = (*stmtite)->Data_Flow_Analysis(id_status);
            stmtite ++;
        }
    }
...
class Exprnode : public Stmtnode{
    ...
    virtual Status  Propagation(data_flow id_status) = 0;
    virtual std::string get_data_id() = 0;
...
class Lvaluenode : public Exprnode{
    ...
    void NOT_traversal(){}
    void getString(){}
    virtual long long get_offset(int nt, std::vector<std::unordered_map<std::string, int> > id_register, long long base_pos) = 0;
...
class Printnode : public Stmtnode{
    ...
    data_flow Data_Flow_Analysis(data_flow id_status){return id_status;}
};
...
class Inputnode : public Stmtnode{
    ...
    std::pair<int, long long> RiscV_generator(int nt, std::vector<std::unordered_map<std::string, int> > id_register, long long base_pos){
        int cnt = ipobj.size();
        long long temp_off = 0;
        for (int i = cnt; i > 0 ; i--){
            temp_off = (ipobj[i-1]->get_offset(nt, id_register, base_pos));
            if (temp_off < 0) printf("\taddi\ta%d,s0,%lld\n", i, temp_off);
            else printf("\taddi\ta%d,a0,%lld\n", i, temp_off*(-1));
        }
        printf("\tlla a0,.LC%lld\n", lcid);
...
class StmtBlocknode : public Stmtnode{
    ...
    std::pair<int, long long> RiscV_generator(int nt, std::vector<std::unordered_map<std::string, int> > id_register, long long base_pos){
        ...
        std::vector<std::unique_ptr<Stmtnode> >::iterator stmtite = stmtDel.begin();
        while(stmtite != stmtDel.end()){
            (*stmtite)->RiscV_generator(0, id_register, base_pos + ex_off);
            stmtite ++;
        }
        if (ex_off != 0) printf("\taddi\tsp,sp,%lld\n", (-1) * ex_off);
        return std::make_pair(0,0);
...
class Ifnode : public Stmtnode{
    ...
    data_flow Data_Flow_Analysis(data_flow id_status){
...
class Binopnode : public Exprnode{
    ...
    std::pair<int, long long> RiscV_generator(int nt, std::vector<std::unordered_map<std::string, int> > id_register, long long base_pos){
        ...
        if (op == ASSIGN){
            ...
            pos = rs->RiscV_generator(nt+1, id_register, 1);
            if (pre.first == 2){
                printf("\tlla t%d,.LC%lld\n", nt, pre.second);
                if (pos.second < 0) printf("\tsd t%d,%lld(s0)\n",nt,  pos.second);
                else if (pos.second > 0) printf("\tsd t%d,%lld(a0)\n", nt, (-1) * pos.second);
    ...
    data_flow Data_Flow_Analysis(data_flow id_status){
        if (op == ASSIGN){
            Status newsts = ls->Propagation(id_status);
            //we actually calculate this twice in RiscV_generator, but it's good enough
            //rs->id --> new status
            std::string target_id = rs->get_data_id();
            if(target_id != "" ){
                std::vector<std::map<std::string, Status> >::reverse_iterator ite = id_status.flow_table.rbegin();
                while(ite != id_status.flow_table.rend()){
                    for (auto i : (*ite)){
                        if (i.first == target_id){
                            (*ite)[i.first] = newsts;
                            return id_status;
                        }
                    }
    ...
    Status Propagation(data_flow id_status){
        Status lsts = ls->Propagation(id_status);
        Status rsts = rs->Propagation(id_status);
        if (lsts.type == _CONSTANT && rsts.type == _CONSTANT) return Status(_CONSTANT, cal_cc(op, lsts.value, rsts.value));
        return Status(_TOP);
    }
    std::string get_data_id(){return "";}
};
...
class Unopnode : public Exprnode{
    ...
    data_flow Data_Flow_Analysis(data_flow id_status){return id_status;}
    Status Propagation(data_flow id_status){
        Status csts = child->Propagation(id_status);
        if (csts.type == _CONSTANT) return Status(_CONSTANT, cal_c(op, csts.value));
        return Status(_TOP);
    }
    std::string get_data_id(){return "";}
};
...
class Lvariablenode : public Lvaluenode{
    ...
    bool isconst;
    Lvariablenode(Token tok):Lvaluenode(ID),_id(tok.key){off_set = 0;const_value = 0; isconst = false;}
    int type_checker(std::vector<std::unordered_map<std::string, std::pair<long long, long long> > > scope_table){
        std::vector<std::unordered_map<std::string, std::pair<long long, long long> > >::reverse_iterator outite = scope_table.rbegin();
        while(outite != scope_table.rend()){
            for (auto i : (*outite)){
    ...
    std::pair<int, long long> RiscV_generator(int nt, std::vector<std::unordered_map<std::string, int> > id_register, long long base_pos){
        if (isconst) return std::make_pair(-1, const_value);
        ...
        while(ite != id_register.rend()){
            for (auto i : (*ite)){
                if (i.first == _id){
                    if (base_pos != 1) printf("\tld t%d,%d(s0)\n",nt, i.second);//ASSIGN cases
                    off_set = i.second;
    ...
    long long get_offset(int nt, std::vector<std::unordered_map<std::string, int> > id_register, long long base_pos){
    ...
    Status Propagation(data_flow id_status){
        ...
        while(ite != id_status.flow_table.rend()){
            for (auto i : (*ite)){
                if (i.first == _id && i.second.type == _CONSTANT){
                    isconst = true;
                    const_value = i.second.value;
                    return Status(_CONSTANT, const_value);
                }else{
                    return i.second;
                }
            }
    ...
    std::string get_data_id(){return _id;}
};
...
class Larraynode : public Lvaluenode{
    ...
    long long _pos;
    std::string _id;
    std::unique_ptr<Exprnode> tpos;
    long long off_set;
    Larraynode(Token id_tok, Token size_tok):Lvaluenode(ID), _id(id_tok.key), _pos(size_tok.value){off_set = 1;tpos = nullptr;}
    Larraynode(Token id_tok, std::unique_ptr<Exprnode> _tpos):Lvaluenode(ID), _id(id_tok.key),tpos(std::move(_tpos)){off_set = 1;_pos = -1;}
    int type_checker(std::vector<std::unordered_map<std::string, std::pair<long long, long long> > > scope_table){
        std::vector<std::unordered_map<std::string, std::pair<long long, long long> > >::reverse_iterator outite = scope_table.rbegin();
        while(outite != scope_table.rend()){
            for (auto i : (*outite)){
                if (i.first == _id){
                    if (tpos == nullptr && (_pos < 0 || _pos >= i.second.second)) throw Out_Of_Boundary_ERROR();
    ...
    std::pair<int, long long> RiscV_generator(int nt, std::vector<std::unordered_map<std::string, int> > id_register, long long base_pos){
        if (off_set < 0) return std::make_pair(0,off_set);
        else if (tpos == nullptr){
        std::vector<std::unordered_map<std::string, int> >::reverse_iterator ite = id_register.rbegin();
        while(ite != id_register.rend()){
            for (auto i : (*ite)){
                if (i.first == _id){
                    if (base_pos != 1) printf("\tld t%d,%lld(s0)\n",nt, (i.second + 8*_pos));//ASSIGN cases
            ...
            while(ite != id_register.rend()){
                for (auto i : (*ite)){
                if (i.first == _id){
                    tpos->RiscV_generator(nt, id_register, 2); // in case t[a] = t[b];
                    printf("\tslli t%d,t%d,3\n", nt, nt);
                    printf("\tadd a0,s0,t%d\n", nt);
                    if (base_pos != 1) printf("\tld t%d,%d(a0)\n",nt, (i.second));//ASSIGN cases
                    //off_set = (i.second);
                    return std::make_pair(0, (i.second) * (-1)); //encode the information a0. 
                }
                }
                ite++;
            }
    ...
    long long get_offset(int nt, std::vector<std::unordered_map<std::string, int> > id_register, long long base_pos){
        ...
        while(ite != id_register.rend()){
            ...
            while(ite != id_register.rend()){
                for (auto i : (*ite)){
                if (i.first == _id){
                    tpos->RiscV_generator(nt, id_register, base_pos);
                    printf("\tslli t%d,t%d,3\n", nt, nt);
                    printf("\tadd a0,s0,t%d\n", nt);
                    //if (base_pos != 1) printf("\tld t%d,%lld(a0)\n",nt, (i.second));//ASSIGN cases
                    //off_set = (i.second);
                    return i.second * (-1); //encode the information a0. 
                }
                }
                ite++;
            }
